...
public class OrdersTest {
    ...
    public void testHasNextFalseList() {
        ...
        orders.addOrder(firstTaco);
        if (!orders.hasNext()) {
            throw new RuntimeException(":( testHasNextFalseList fail: Orders should have one order in them.");
        }
        TacoImpl closedTaco = orders.closeNextOrder();
        if (closedTaco == null) {
            throw new RuntimeException(":( testHasNextFalseList fail: closed taco should not be null.");
        }
        if (!closedTaco.equals(firstTaco)) {
            throw new RuntimeException(":( testHasNextFalseList fail: This should truely never happen.");
        }
        if (orders.hasNext()) {
            throw new RuntimeException(":( testHasNextFalseList fail: Orders should be empty.");
        }
        System.out.println(":) testHasNextFalseList passed");
    }
    ...
    public void testCloseNextOrderList() {
        ...
        TacoImpl shouldBeFirstTaco = orders.closeNextOrder();
        if (orders.howManyOrders() != 1) {
            throw new RuntimeException(":( testCloseNextOrderList Failed: orders should only have one order left");
        }
        if (firstTaco.equals(shouldBeFirstTaco)){
            System.out.println(":) testCloseNextOrderList passed");
        } else {
            throw new RuntimeException(":( testCloseNextOrderList Failed: should be the first taco was first out.");
        }
    }
}
